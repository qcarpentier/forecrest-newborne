...
import { Card, NumberField, PageLayout, Button, KpiCard, SearchInput, FilterDropdown, DevVal, ButtonUtility, InfoTip, DataTable, Badge, ExportButtons, AlertCard, FinanceLink, StackedBar } from "../../components";
...
var TABS = ["results", "pcmn", "depreciation", "vat", "tax", "allocation"];
...
function AdviceCard({ title, desc, priorityLabel, color, children }) {
  var badgeColor = color === "success" ? "success" : color === "warning" ? "warning" : color === "error" ? "error" : "brand";
  return (
    <div style={{
      padding: "var(--sp-4)", background: "var(--bg-card)", borderRadius: "var(--r-lg)",
      border: "1px solid var(--border)",
    }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "var(--sp-2)" }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text-primary)" }}>{title}</div>
        {priorityLabel ? (
          <Badge color={badgeColor} size="sm">{priorityLabel}</Badge>
        ) : null}
      </div>
...
export default function AccountingPage({ costs, sals, cfg, debts, streams, stocks, assets, totalRevenue, monthlyCosts, opCosts, salCosts, ebit, isoc, isocR, isocS, isocEff, netP, resLeg, annVatC, annVatD, vatBalance, esopMonthly, esopEnabled, setCosts, onNavigate, onRandomizeAll, chartPalette }) {
  var tAll = useT();
  ...
  var [depSearch, setDepSearch] = useState("");
  var [depClassFilter, setDepClassFilter] = useState("all");
  ...
  // ── 3. Balance sheet (depreciation from assets) ──
  ...
  (assets || []).forEach(function (a, ai) {
    ...
    var annDepr = depreciable / a.years;
    var monthly = annDepr / 12;
    fixedAssetsGross += a.amount;
  ...
  var TAB_LABELS = {
    ...
    vat: t.tab_vat || "TVA",
    tax: t.tab_tax || (lk === "fr" ? "Impôts" : "Taxes"),
    allocation: t.tab_allocation || (lk === "fr" ? "Affectation" : "Allocation"),
  };
  var TAB_COUNTS = {
    results: null,
    ...
    <PageLayout title={t.title} subtitle={t.subtitle} icon={BookOpen} iconColor="var(--text-muted)">
        ...
        <div>
          <DataTable
            data={(function () {
              var items = depItems;
              if (depSearch.trim()) {
                var q = depSearch.trim().toLowerCase();
                items = items.filter(function (d) { return (d.label || "").toLowerCase().indexOf(q) >= 0 || (d.pcmn || "").indexOf(q) >= 0; });
              }
              if (depClassFilter !== "all") {
                items = items.filter(function (d) { return (d.pcmn || "").startsWith(depClassFilter); });
              }
              return items;
            })()}
            columns={[
              {
                id: "label", header: t.depr_item,
                enableSorting: true, meta: { align: "left", minWidth: 200, grow: true },
                accessorFn: function (row) { return row.label; },
                cell: function (info) {
                  var d = info.row.original;
                  return (
                    <div style={{ display: "flex", alignItems: "center", gap: "var(--sp-2)" }}>
                      <Badge color="gray" size="sm">{d.pcmn}</Badge>
                      <span style={{ fontWeight: 500 }}>{d.label || "—"}</span>
                    </div>
                  );
                },
                footer: function () { return <span style={{ fontWeight: 600 }}>{t.depr_total}</span>; },
              },
              {
                id: "acquisition", header: t.depr_acquisition,
                enableSorting: true, meta: { align: "right" },
                accessorFn: function (row) { return row.acquisition; },
                cell: function (info) { return <span style={{ fontVariantNumeric: "tabular-nums" }}>{eur(info.getValue())}</span>; },
                footer: function () { return <span style={{ fontWeight: 600, fontVariantNumeric: "tabular-nums" }}>{eur(depItems.reduce(function (s, d) { return s + d.acquisition; }, 0))}</span>; },
              },
              {
                id: "years", header: t.depr_duration,
                enableSorting: true, meta: { align: "right" },
                accessorFn: function (row) { return row.years; },
                cell: function (info) {
                  return <span style={{ fontVariantNumeric: "tabular-nums" }}>{info.getValue()} {lk === "fr" ? "ans" : "yrs"}</span>;
                },
              },
              {
                id: "method", header: lk === "fr" ? "Méthode" : "Method",
                enableSorting: true, meta: { align: "left" },
                accessorFn: function (row) { return row.method; },
                cell: function (info) {
                  var v = info.getValue();
                  return <Badge color={v === "declining" ? "warning" : "info"} size="sm">{v === "declining" ? (lk === "fr" ? "Dégressif" : "Declining") : (lk === "fr" ? "Linéaire" : "Linear")}</Badge>;
                },
              },
              {
                id: "annual", header: t.depr_annual,
                enableSorting: true, meta: { align: "right" },
                accessorFn: function (row) { return row.annual; },
                cell: function (info) { return <span style={{ fontWeight: 600, fontVariantNumeric: "tabular-nums" }}>{eur(info.getValue())}</span>; },
                footer: function () { return <span style={{ fontWeight: 700, fontVariantNumeric: "tabular-nums" }}>{eur(depItems.reduce(function (s, d) { return s + d.annual; }, 0))}</span>; },
              },
              {
                id: "monthly", header: t.depr_monthly,
                enableSorting: true, meta: { align: "right" },
                accessorFn: function (row) { return row.monthly; },
                cell: function (info) { return <span style={{ fontVariantNumeric: "tabular-nums", color: "var(--text-muted)" }}>{eur(info.getValue())}</span>; },
                footer: function () { return <span style={{ fontWeight: 600, fontVariantNumeric: "tabular-nums", color: "var(--text-muted)" }}>{eur(depItems.reduce(function (s, d) { return s + d.monthly; }, 0))}</span>; },
              },
            ]}
            toolbar={
              <>
                <div style={{ display: "flex", gap: "var(--sp-2)", alignItems: "center", flexWrap: "wrap" }}>
                  <SearchInput value={depSearch} onChange={setDepSearch} placeholder={lk === "fr" ? "Rechercher…" : "Search…"} />
                  <FilterDropdown value={depClassFilter} onChange={setDepClassFilter} options={[
                    { value: "all", label: lk === "fr" ? "Toutes les classes" : "All classes" },
                    { value: "21", label: lk === "fr" ? "21 — Immobilisations incorporelles" : "21 — Intangible assets" },
                    { value: "22", label: lk === "fr" ? "22 — Terrains et constructions" : "22 — Land & buildings" },
                    { value: "23", label: lk === "fr" ? "23 — Installations" : "23 — Plant & equipment" },
                    { value: "24", label: lk === "fr" ? "24 — Mobilier et matériel" : "24 — Furniture & equipment" },
                    { value: "25", label: lk === "fr" ? "25 — Immobilisations en cours" : "25 — Assets in progress" },
                  ]} />
                </div>
                <div style={{ display: "flex", gap: "var(--sp-2)", alignItems: "center" }}>
                  <ExportButtons columns={[{ id: "label", header: t.depr_item }, { id: "pcmn", header: "PCMN" }, { id: "acquisition", header: t.depr_acquisition }, { id: "years", header: t.depr_duration }, { id: "method", header: lk === "fr" ? "Méthode" : "Method" }, { id: "annual", header: t.depr_annual }, { id: "monthly", header: t.depr_monthly }]} data={depItems} filename="amortissements" />
                </div>
              </>
            }
            emptyState={
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "var(--sp-3)" }}>
                ...
                <div style={{ fontSize: 14, fontWeight: 600, color: "var(--text-primary)" }}>{t.empty_depr_title}</div>
                <div style={{ fontSize: 13, color: "var(--text-muted)", maxWidth: 320, textAlign: "center" }}>{t.empty_depr_desc}</div>
              </div>
            }
            emptyMinHeight={200}
            scrollable
      ...
      {activeTab === "vat" ? (
        <div>
          ...
          <div className="resp-grid-3" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "var(--gap-md)", marginBottom: "var(--gap-lg)" }}>
            <KpiCard label={t.vat_collected} value={eurShort(annVatC)} fullValue={eur(annVatC)} />
            <KpiCard label={t.vat_deductible} value={eurShort(annVatD)} fullValue={eur(annVatD)} />
            <KpiCard label={t.vat_balance} value={eurShort(Math.abs(vatBalance))} fullValue={eur(Math.abs(vatBalance))} sub={vatBalance >= 0 ? t.vat_due : t.vat_credit} />
          </div>

          {vatBalance > 0 ? (
            <div style={{ marginBottom: "var(--gap-md)" }}>
              <AlertCard color="warning" title={lk === "fr" ? "TVA à reverser" : "VAT due"}>
                {lk === "fr"
                  ? <span>Vous devez reverser <strong>{eur(vatBalance)}</strong> au SPF Finances. Ce montant est la différence entre la <FinanceLink term="tva_collectee" label="TVA collectée" desc={t.vat_tip_declaration} /> sur vos ventes et la <FinanceLink term="tva_deductible" label="TVA déductible" desc={t.vat_tip_credit} /> sur vos achats.</span>
                  : <span>You owe <strong>{eur(vatBalance)}</strong> to the tax authority. This is the difference between VAT collected on sales and VAT deductible on purchases.</span>}
              </AlertCard>
            </div>
          ) : null}
            ...
            {annVatC > 0 || annVatD > 0 ? (
              <div style={{ display: "flex", flexDirection: "column", gap: "var(--sp-2)" }}>
                {[1, 2, 3, 4].map(function (q) {
                  var qVatC = annVatC / 4;
                  var qVatD = annVatD / 4;
                  var qBalance = qVatC - qVatD;
                  var deadlines = { 1: lk === "fr" ? "20 avril" : "April 20", 2: lk === "fr" ? "20 juillet" : "July 20", 3: lk === "fr" ? "20 octobre" : "October 20", 4: lk === "fr" ? "20 janvier (N+1)" : "January 20 (N+1)" };
                  return (
                    <div key={q} style={{ display: "flex", alignItems: "center", gap: "var(--sp-3)", padding: "var(--sp-2) var(--sp-3)", background: q % 2 === 0 ? "var(--bg-accordion)" : "transparent", borderRadius: "var(--r-md)", fontSize: 12 }}>
                      <span style={{ fontWeight: 600, color: "var(--text-primary)", minWidth: 24 }}>T{q}</span>
                      <span style={{ color: "var(--text-muted)", flex: 1 }}>{deadlines[q]}</span>
                      <span style={{ fontVariantNumeric: "tabular-nums", color: "var(--text-secondary)", minWidth: 80, textAlign: "right" }}>{eur(qVatC)}</span>
                      <span style={{ fontVariantNumeric: "tabular-nums", color: "var(--text-secondary)", minWidth: 80, textAlign: "right" }}>−{eur(qVatD)}</span>
                      <span style={{ fontWeight: 600, fontVariantNumeric: "tabular-nums", color: qBalance >= 0 ? "var(--color-error)" : "var(--color-success)", minWidth: 80, textAlign: "right" }}>{eur(qBalance)}</span>
                    </div>
                  );
                })}
              </div>
            ) : (
              <div style={{ fontSize: 13, color: "var(--text-faint)", textAlign: "center", padding: "var(--sp-4)" }}>{t.vat_empty}</div>
            )}
          </Card>
        </div>
      ) : null}
      ...
      {activeTab === "tax" ? (
        <div>
          {/* KPIs */}
          <div className="resp-grid-4" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "var(--gap-md)", marginBottom: "var(--gap-lg)" }}>
            <KpiCard label={t.tax_kpi_base} value={ebit > 0 ? eurShort(ebit) : "—"} fullValue={ebit > 0 ? eur(ebit) : undefined} glossaryKey="ebitda" />
            <KpiCard label={t.tax_kpi_total} value={isoc > 0 ? eurShort(isoc) : "—"} fullValue={isoc > 0 ? eur(isoc) : undefined} />
            <KpiCard label={t.tax_kpi_rate} value={isocEff > 0 ? (isocEff * 100).toFixed(1) + "%" : "—"} />
            <KpiCard label={t.tax_kpi_net} value={eurShort(netP)} fullValue={eur(netP)} />
          </div>
          ...
          <Card sx={{ padding: "var(--sp-4)", marginBottom: "var(--gap-lg)" }}>
            ...
            {ebit > 0 ? (
              <div style={{ display: "flex", flexDirection: "column", gap: "var(--sp-3)" }}>
                <StackedBar
                  data={{ reduced: isocR, normal: isocS > 0 ? isocS : 0 }}
                  labels={{ reduced: t.tax_bracket_reduced + " (20%)", normal: t.tax_bracket_normal + " (25%)" }}
                  palette={chartPalette}
                  formatter={function (v) { return eur(v); }}
                />
                <div className="resp-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--sp-3)" }}>
                  <div style={{ padding: "var(--sp-3)", background: "var(--bg-accordion)", borderRadius: "var(--r-md)" }}>
                    <div style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 4 }}>{t.tax_bracket_reduced} — 20%</div>
                    <div style={{ fontSize: 11, color: "var(--text-faint)" }}>{t.tax_bracket_amount}</div>
                    <div style={{ fontSize: 16, fontWeight: 700, fontFamily: "'Bricolage Grotesque', sans-serif", marginTop: 4 }}>{eur(isocR)}</div>
                  </div>
                  <div style={{ padding: "var(--sp-3)", background: "var(--bg-accordion)", borderRadius: "var(--r-md)" }}>
                    <div style={{ fontSize: 11, color: "var(--text-muted)", marginBottom: 4 }}>{t.tax_bracket_normal} — 25%</div>
                    <div style={{ fontSize: 11, color: "var(--text-faint)" }}>{t.tax_bracket_excess}</div>
                    <div style={{ fontSize: 16, fontWeight: 700, fontFamily: "'Bricolage Grotesque', sans-serif", marginTop: 4 }}>{eur(isocS)}</div>
                  </div>
                </div>
                <div style={{ fontSize: 11, color: "var(--text-faint)", lineHeight: 1.5 }}>
                  <FinanceLink term="isoc" label={lk === "fr" ? "Taux réduit PME" : "Reduced SME rate"} desc={t.tax_tip_pme} /> — {t.tax_tip_pme}
                </div>
              </div>
            ) : (
              <div style={{ fontSize: 13, color: "var(--text-faint)", textAlign: "center", padding: "var(--sp-4)" }}>
                {lk === "fr" ? "Pas de base imposable positive." : "No positive tax base."}
              </div>
            )}
          </Card>
          ...
          <Card sx={{ padding: "var(--sp-4)" }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: "var(--sp-3)" }}>
              {t.tax_prepayments_title}
            </div>
            <div style={{ fontSize: 12, color: "var(--text-secondary)", lineHeight: 1.5, marginBottom: "var(--sp-3)" }}>
              {t.tax_prepayments_desc}
            </div>
            {isoc > 0 ? (
              <div style={{ display: "flex", flexDirection: "column", gap: "var(--sp-2)" }}>
                {[
                  { label: t.tax_q1, pct: 0.25 },
                  { label: t.tax_q2, pct: 0.25 },
                  { label: t.tax_q3, pct: 0.25 },
                  { label: t.tax_q4, pct: 0.25 },
                ].map(function (q, i) {
                  var amount = isoc * q.pct;
                  return (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: "var(--sp-3)", padding: "var(--sp-2) var(--sp-3)", background: i % 2 === 0 ? "var(--bg-accordion)" : "transparent", borderRadius: "var(--r-md)", fontSize: 12 }}>
                      <span style={{ color: "var(--text-muted)", flex: 1 }}>{q.label}</span>
                      <span style={{ fontWeight: 600, fontVariantNumeric: "tabular-nums", color: "var(--text-primary)" }}>{eur(amount)}</span>
                    </div>
                  );
                })}
              </div>
            ) : (
              <div style={{ fontSize: 13, color: "var(--text-faint)", textAlign: "center", padding: "var(--sp-2)" }}>
                {lk === "fr" ? "Aucun versement anticipé nécessaire." : "No advance payment needed."}
              </div>
            )}
          </Card>
        </div>
      ...
      {/* ── Tab: Profit Allocation ── */}
      {activeTab === "allocation" ? (
        <div>
          {/* KPIs */}
          <div className="resp-grid-3" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "var(--gap-md)", marginBottom: "var(--gap-lg)" }}>
            <KpiCard label={t.alloc_kpi_net} value={eurShort(resultNet)} fullValue={eur(resultNet)} />
            <KpiCard label={t.alloc_kpi_reserve} value={eurShort(resLeg)} fullValue={eur(resLeg)} />
            <KpiCard label={t.alloc_kpi_distributable} value={eurShort(distributable)} fullValue={eur(distributable)} />
          </div>

          {resultNet <= 0 ? (
            <AlertCard color="warning" title={lk === "fr" ? "Pas de bénéfice" : "No profit"}>
              {t.alloc_no_profit}
            </AlertCard>
          ) : (
            <div>
              ...
              <Card sx={{ padding: "var(--sp-4)", marginBottom: "var(--gap-lg)" }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: "var(--sp-3)" }}>
                  {t.alloc_breakdown_title}
                </div>
                <StackedBar
                  data={{
                    reserve: resLeg,
                    dividends: distributable * 0.3,
                    bonus: Math.min(distributable * 0.1, resultNet * 0.3),
                    retained: distributable - distributable * 0.3 - Math.min(distributable * 0.1, resultNet * 0.3),
                  }}
                  labels={{
                    reserve: t.alloc_reserve_label,
                    dividends: t.alloc_dividend_label,
                    bonus: t.alloc_bonus_label,
                    retained: t.alloc_retained_label,
                  }}
                  palette={chartPalette}
                  formatter={function (v) { return eur(v); }}
                />
              ...
              {/* Advice cards — moved from old TVA tab */}
              <div className="resp-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--gap-md)" }}>
                ...
                <AdviceCard title={t.advice_capital_title} desc={t.advice_capital_desc} priorityLabel={equityRatio < 0.3 ? t.advice_high : t.advice_low} color={equityRatio < 0.3 ? "warning" : "success"}>
                  <AdviceRow label={t.advice_capital_solvency} value={pct(equityRatio)} bold color={equityRatio >= 0.3 ? "var(--color-success)" : "var(--color-warning)"} />
