...
export default function KpiCard({ label, value, fullValue, sub, color, icon, spark, sparkColor, target, current, onClick, tip, glossaryKey }) {
  ...
  return (
    <Card onClick={onClick} sx={{ display: "flex", flexDirection: "column", gap: "var(--sp-2)", position: "relative", cursor: onClick ? "pointer" : undefined, borderLeft: hit ? "3px solid var(--color-success)" : progress !== null ? "3px solid var(--color-warning)" : undefined }}>
      <div style={{ display: "flex", alignItems: "center", gap: "var(--sp-2)" }}>
        {icon ? <span style={{ color: "var(--text-muted)", display: "flex" }}>{icon}</span> : null}
        <span style={{ fontSize: 12, color: "var(--text-muted)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em", flex: 1 }}>{label}</span>
        {spark}
      </div>
      <div
        onMouseEnter={hasTooltip ? function () { setHover(true); } : undefined}
        onMouseLeave={hasTooltip ? function () { setHover(false); } : undefined}
        style={{ fontSize: 30, fontWeight: 800, color: color || "var(--text-primary)", lineHeight: 1.1, letterSpacing: "-0.03em", position: "relative", cursor: hasTooltip ? "help" : undefined, fontFamily: "'Bricolage Grotesque', 'DM Sans', sans-serif" }}
      >
        {String(value)}
        {hasTooltip && hover ? (
          <span style={{
      ...
      {sub ? <div style={{ fontSize: 11, color: "var(--text-faint)", lineHeight: 1.45 }}>{sub}</div> : null}
      {tip ? <div style={{ fontSize: 11, color: "var(--text-faint)", lineHeight: 1.45 }}>{tip}</div> : null}
